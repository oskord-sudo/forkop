...
function section_uses_singbox(section) {
    ...
    if (connections.is_connections_action(action)) {
        // Proxy links / subscriptions / JSON — sing-box
        if (section_has_proxy_transport(section))
            return true;
        // Interface-only (AWG/WG): kernel path, no sing-box outbound
        return false;
    }
    return false;
}
...
function add_outbound_for_section(config, section, taken, sections) {
    ...
    // Pure VPN / interface-only: kernel policy routing, no sing-box outbounds
    if (action == "vpn" || action == "zapret" || action == "zapret2")
        return;
    if (connections.is_connections_action(action)) {
        // Interface-only "connection" (no proxy URLs/subs/JSON) → same as VPN in economy
        if (!section_has_proxy_transport(section) && length(connections.interfaces(section)) > 0)
            return;
        if (!section_has_proxy_transport(section) && length(connections.interfaces(section)) == 0 &&
